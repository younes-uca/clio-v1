...
@Component({
  selector: 'app-purchase-request-edit-admin',
  templateUrl: './purchase-request-edit-admin.component.html'
})
export class PurchaseRequestEditAdminComponent extends AbstractEditController<PurchaseRequestDto, PurchaseRequestCriteria, PurchaseRequestService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.datePurchaseRequest = this.purchaseRequestService.format(this.item.datePurchaseRequest);
        this.item.dateLivraison = this.purchaseRequestService.format(this.item.dateLivraison);
    }



    public validatePurchaseRequestProducts(){
   ...
   public addPurchaseRequestProducts() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.purchaseRequestProductsElement.id == null){
                this.item.purchaseRequestProducts.push(this.purchaseRequestProductsElement);
            }else{
                const index = this.item.purchaseRequestProducts.findIndex(e => e.id == this.purchaseRequestProductsElement.id);
                this.item.purchaseRequestProducts[index] = this.purchaseRequestProductsElement;
            }
